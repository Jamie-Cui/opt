...
_clone_repos:
	...
	echo '  case "$$line" in'; \
	echo '    "[repo."*) repo=$$(echo "$$line" | sed "s/\[repo\.//; s/\]//");;'; \
	...
	echo '      if [ -n "$$branch" ]; then clone_args="--branch $$branch $$clone_args"; fi'; \
	echo '      if [ -n "$$depth" ]; then clone_args="--depth $$depth $$clone_args"; fi'; \
	echo '      git clone $$clone_args'; \
	echo '      cloned=true'; \
	echo '      if [ -n "$$tag" ]; then'; \
	echo '        echo "  Checking out tag $$tag..."'; \
	echo '        git -C "$$dir" checkout -q "$$tag"'; \
	echo '      fi'; \
